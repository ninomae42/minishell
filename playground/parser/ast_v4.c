...
t_node	*simple_command_element_choice(t_parser *parser);
...
t_node	*pipeline(t_parser *parser)
{
	...
	while (parser->token->type != TK_EOF)
	{
		if (parser->token->type != TK_EOF)
		{
			...
			// node = new_node(ND_PIPELINE, simple_command(parser), node);
		}
		else
			return (node);
	}
...
t_node	*simple_command(t_parser *parser)
{
	...
	while (parser->token->type != TK_EOF && strcmp(parser->token->literal, "|") != 0)
	{
		if (parser->token->type != TK_EOF)
...
t_node	*simple_command_element(t_parser *parser)
{
	...
	// printf("current: %d, litreal: %s\n", parser->token->type, parser->token->literal);
	node = simple_command_element_choice(parser);
	// while (parser->token->type != TK_EOF)
	while (1)
	{
	// printf("current: %d, litreal: %s\n", parser->token->type, parser->token->literal);
		if (parser->token->type != TK_EOF && strcmp(parser->token->literal, "|") != 0)
			node = new_node(ND_SIMPLE_COMMAND_ELEMENT, node, simple_command_element_choice(parser));
		else
			// return (node);
...
t_node	*simple_command_element_choice(t_parser *parser)
{
	...
	// puts("simple_command_element_choice");
